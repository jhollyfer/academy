...
@inject()
export default class PhotoShowUseCase {
  async execute(payload: Payload): Promise<Response> {
    try {
      ...
      logger.error({ err: error }, '[photos > show][error]')
      return left(HTTPException.InternalServerError('Erro interno do servidor', 'PHOTO_SHOW_ERROR'))
    }
  }
}
