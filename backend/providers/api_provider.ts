...
/**
 * A única asserção `as` que fica no backend, e ela é limite de linguagem.
 *
 * `serialize` do `ApiSerializer` é genérica: o tipo do retorno é calculado a
 * partir do `Data` que entra. `Object.assign` não reconstrói assinatura
 * genérica - ele infere a função concreta que o wrapper implementa, e o
 * `satisfies` recusa (TS1360, `Promise<unknown>` contra o retorno mapeado).
 *
 * O tipo certo existe e é exatamente o declarado abaixo; o que falta é uma
 * forma de o compilador chegar nele por inferência. Enquanto faltar, a
 * asserção é o que faz `context.serialize(...)` continuar tipado no controller.
 */
